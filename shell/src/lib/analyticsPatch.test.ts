...
test("uses field__in with a comma-joined value for an array cross-filter value", () => {
  const ctx: AnalyticsContextState = {
    ...EMPTY,
    crossFilter: {
      "ds-1": { field: "region", value: ["Nord", "Sud"], originSourceId: "src-OTHER" },
    },
  };
...
test("uses field__gte/field__lte for a range cross-filter value", () => {
  const ctx: AnalyticsContextState = {
    ...EMPTY,
    crossFilter: {
      "ds-1": { field: "score", value: { from: "10", to: "50" }, originSourceId: "src-OTHER" },
    },
  };
  expect(derivePatch(source, ctx, { "ds-1": dataset })).toEqual({
    score__gte: "10",
...
test("excludes a range cross-filter patch when this source is the origin", () => {
  const ctx: AnalyticsContextState = {
    ...EMPTY,
    crossFilter: {
      "ds-1": { field: "score", value: { from: "10", to: "50" }, originSourceId: "src-1" },
    },
  };
  expect(derivePatch(source, ctx, { "ds-1": dataset })).toEqual({});
});
...
test("translates a spatial/bbox link into a bbox patch derived from the entry's geometry", () => {
  ...
  const ctx: AnalyticsContextState = {
    ...EMPTY,
    crossFilter: {
      "ds-2": { field: "commune", value: "Brive", originSourceId: "src-OTHER", geometry: polygon },
    },
  };
  expect(
    derivePatch(source, ctx, {
      "ds-1": { ...dataset, reactsToExtent: false },
      "ds-2": spatialLinked,
    }),
  ).toEqual({
    bbox: "2,48,3,49",
