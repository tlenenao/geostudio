...
function useKpiComparison(
  ...
  const valueSource: DataSource | null =
    active && referencePeriod
      ? windowedStatisticsSource(
          originSourceId,
          datasetId as string,
          dataset as DatasetConfig,
          analyticsCtx,
          timeRange as { from: string; to: string },
          { agg, field: field || undefined },
        )
      : null;
  ...
  const referenceRange =
    active && referencePeriod
      ? referenceWindow(timeRange as { from: string; to: string }, referencePeriod)
      : null;
  ...
  const sparklineSource: DataSource | null =
    active && sparklineEnabled
      ? windowedStatisticsSource(
          originSourceId,
          datasetId as string,
          dataset as DatasetConfig,
  ...
  const deltaPct =
    delta !== null && reference !== null && reference !== 0 ? delta / reference : null;
  const sparklinePoints =
    sparklineEnabled && dataset?.timeField && sparklineQuery.data
      ? sparklineQuery.data.map((r) => ({
          bucket: String(r.properties[dataset.timeField as string] ?? ""),
...
export function registerIndicatorWidget(): void {
  registerWidget({
    ...
    Component: ({ props, ctx }) => {
      ...
      const badge =
        comparison.active && referencePeriod && comparison.delta !== null
          ? deltaLabel(comparison.delta, comparison.deltaPct, referencePeriod)
          : null;

      const level =
        criticalWhen || warningWhen
