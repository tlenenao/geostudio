...
def env():
    ...
    with Session() as session:
        ...
        session.add(Collection(id="col-1", tenant_id=tenant.id, owner_id=owner.id,
                                table_name="col_1", title="Col 1", pk_column="id"))
        session.commit()
...
def test_item_share_does_not_leak_to_collections(env):
    ...
    session.add(Item(id="col-1", tenant_id=tenant.id, owner_id=owner.id,
                      resource_type="app", title="Item col-1"))
