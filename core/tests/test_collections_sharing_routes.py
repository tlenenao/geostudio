...
def env():
    ...
    with Session() as s:
        ...
        group = Group(id=uuid.uuid4().hex, tenant_id=tenant.id, name="equipe", created_by=admin.id)
        s.add(group)
        s.flush()
...
def test_put_sharing_cross_tenant_group_is_404(env):
    ...
    with Session() as s:
        ...
        foreign_group = Group(
            id=uuid.uuid4().hex, tenant_id=other_tenant.id, name="ailleurs", created_by=outsider.id
        )
        s.add(foreign_group)
