...
def test_create_collection_enqueues_an_embedding_job(session, tenant_and_user, monkeypatch):
    ...
    monkeypatch.setattr(
        collection_jobs.embed_collection_task, "defer",
...
def test_list_visible_collections_hybrid_search_ranks_semantic_match_ahead_of_weak_text_match(
    ...
    semantically_close = repo.create_collection(
        pg_session, tenant_id=tenant.id, owner_id=user.id, table_name="c_close",
        title="Sujet totalement différent", description="", is_public=True,
        pk_column="id", geometry_column=None, geometry_type=None, srid=None,
    )
    semantically_close.embedding = close_vector
    weak_text_match = repo.create_collection(
        pg_session, tenant_id=tenant.id, owner_id=user.id, table_name="c_weak",
        title="incidents", description="", is_public=True,
        pk_column="id", geometry_column=None, geometry_type=None, srid=None,
    )
    weak_text_match.embedding = far_vector
    repo.create_collection(
        pg_session, tenant_id=tenant.id, owner_id=user.id, table_name="c_filler",
        title="incidents voirie", description="", is_public=True,
        pk_column="id", geometry_column=None, geometry_type=None, srid=None,
    )
    pg_session.flush()
    ...
    cols = repo.list_visible_collections(
        pg_session, tenant_id=tenant.id, user_id=user.id, is_admin=False, q="incidents voirie",
    )
