...
def test_create_share_link_requires_write_access(client):
    response = client.post(f"/items/{client.item_id}/share-links", json={"ttlDays": 7})
...
def test_create_share_link_refuses_a_ttl_above_the_server_cap(client):
    response = client.post(f"/items/{client.item_id}/share-links", json={"ttlDays": 31})
    assert response.status_code == 422


def test_create_share_link_on_unreadable_item_returns_404(client):
    response = client.post("/items/does-not-exist/share-links", json={"ttlDays": 7})
...
def test_list_share_links_shows_status(client):
    client.post(f"/items/{client.item_id}/share-links", json={"ttlDays": 7})
    listed = client.get(f"/items/{client.item_id}/share-links")
...
def test_revoke_share_link(client):
    client.post(f"/items/{client.item_id}/share-links", json={"ttlDays": 7})
    link_id = client.get(f"/items/{client.item_id}/share-links").json()[0]["id"]
    response = client.delete(f"/items/{client.item_id}/share-links/{link_id}")
    assert response.status_code == 204
    rows = client.get(f"/items/{client.item_id}/share-links").json()
    assert rows[0]["revoked"] is True


def test_revoke_unknown_share_link_returns_404(client):
    response = client.delete(f"/items/{client.item_id}/share-links/nope")
    assert response.status_code == 404


def test_resolve_share_link_by_token(client):
    created = client.post(f"/items/{client.item_id}/share-links", json={"ttlDays": 7}).json()
    ...
    response = client.get(f"/share-links/{token}")
...
def test_resolve_unknown_token_returns_401(client):
    response = client.get("/share-links/not-a-real-token")
...
def test_resolve_revoked_link_returns_401_even_before_expiry(client):
    ...
    token = created["url"].rsplit("/", 1)[-1]
    link_id = client.get(f"/items/{client.item_id}/share-links").json()[0]["id"]
    client.delete(f"/items/{client.item_id}/share-links/{link_id}")
