...
@pytest.mark.postgis
def test_arcgis_reference_creates_external_items_and_reharvest_no_duplicate(
    ...
    rec = harvest_repo.get_record(
        pg_session, tenant_id=tenant.id, source_id=source.id, external_id=f"{SERVICE}/0"
    )
...
@pytest.mark.postgis
def test_arcgis_copy_creates_local_collection_with_full_paginated_geojson(
    pg_session, pg_tenant_and_user, monkeypatch
):
    tenant, user = pg_tenant_and_user
    monkeypatch.setattr(service, "get_connector", lambda t: _arcgis_connector())
    ...
    collection = collections_repo.get_collection(
        pg_session, tenant_id=tenant.id, collection_id=rec.collection_id
    )
