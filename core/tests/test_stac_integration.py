...
def _seed(app, client, public):
    client.post("/v1/collections", json={"tableName": "stac_roads", "isPublic": public})
    for lon, lat in [(1.0, 44.0), (2.0, 45.0), (3.0, 46.0)]:
        client.post(
            "/v1/collections/stac_roads/items",
            json={
                "type": "Feature",
                "properties": {"n": "x"},
...
def test_full_stac_navigation(pg_app):
    ...
    landing = client.get("/v1/stac").json()
    ...
    items = client.get("/v1/stac/collections/stac_roads/items?limit=2").json()
    ...
    tight = client.get("/v1/stac/collections/stac_roads/items?bbox=0.9,43.9,1.1,44.1").json()
    assert len(tight["features"]) == 1

    search = client.get("/v1/stac/search?collections=stac_roads&bbox=0,40,4,47").json()
...
def test_anonymous_sees_public_only_no_leak(pg_app):
    ...
    assert client.get("/v1/stac/collections/stac_roads").status_code == 404
    assert client.get("/v1/stac/collections/stac_roads/items").status_code == 404
    assert client.get("/v1/stac/collections").json()["collections"] == []
    assert client.get("/v1/stac/search").json()["features"] == []
    # Une collection inexistante renvoie le même 404.
    assert client.get("/v1/stac/collections/does-not-exist").status_code == 404
