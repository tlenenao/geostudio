...
|---|---|---|---|---|
| **SP-44** | Débloquer M14 : exécution réelle des 5 tests `@pytest.mark.qgis` | GAP-01 | Prérequis : sidecar QGIS réel disponible (conteneur `qgis-worker`, profil `etl`, `CORE_TEST_QGIS_WORKER_URL` câblé en CI ou en session manuelle). **Bloque l'étape 9 de SP-43** (découpage `pipelines/runtime.py`) — doit être fait avant elle, pas après. Seul gap bloquant de toute la revue. | 1-3 |
