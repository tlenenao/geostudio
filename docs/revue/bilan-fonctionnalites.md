...
307 fonctionnalités — santé médiane 96.0 sur 307 mesurables.
...
| 11 | **Fédération des données** | 93.5 |
...
| Cartographie | Générer et publier des tuiles vectorielles pré-calculées (PMTiles) depuis PostGIS vers MinIO | `cartographie-generer-et-publier-des-tuiles-vectorielles-pre-calculees-pmtiles-de` | 36.0 | = | basse | 0.0 | — | — | 90.0 |
...
| Cartographie | Uploader une icône SVG personnalisée dans une bibliothèque d'icônes du tenant | `cartographie-uploader-une-icone-svg-personnalisee-dans-une-bibliotheque-d-icones` | 84.2 | = | moyenne | 95.5 | 100.0 | 50.0 | 90.0 |
...
| auth | Garde : refus de démarrer en mode mock d'authentification hors développement | `auth-garde-refus-de-demarrer-en-mode-mock-d-authentification-hors-developpement` | 95.6 | = | haute | 99.4 | — | — | 90.0 |
...
| Carte | Catalogue de sources de couches (collections du cœur, couches raster moissonnées, tuiles 3D hébergées) | `carte-catalogue-de-sources-de-couches-collections-du-cur-couches-raster-moissonn` | 96.0 | = | haute | 100.0 | — | — | 90.0 |
| Carte | Persistance de la configuration de carte (basemap, vue, couches, terrain, mise en page d'impression) | `carte-persistance-de-la-configuration-de-carte-basemap-vue-couches-terrain-mise-` | 96.0 | = | haute | 100.0 | — | — | 90.0 |
...
| sharing | Partager une collection avec des groupes, avec court-circuit admin | `sharing-partager-une-collection-avec-des-groupes-avec-court-circuit-admin` | 94.7 | = | moyenne | 95.7 | 100.0 | 100.0 | 80.0 |
| Carte | Visualisations deck.gl agrégées (heatmap/hexbin/column) | `carte-visualisations-deck-gl-agregees-heatmap-hexbin-column` | 94.9 | = | moyenne | 98.2 | — | — | 90.0 |
...
| Carte | Persistance de collectionId/pkColumn sur une couche 'feature' (URL GeoJSON) | `carte-persistance-de-collectionid-pkcolumn-sur-une-couche-feature-url-geojson` | 96.0 | = | moyenne | 100.0 | — | — | 90.0 |
...
| Fédération des données | Couches raster externes moissonnées (WMS/WMTS) proposées dans le sélecteur de couches de la carte | `federation-des-donnees-couches-raster-externes-moissonnees-wms-wmts-proposees-da` | 96.0 | = | moyenne | 100.0 | — | — | 90.0 |
...
| Catalogue/Items | Historique de versions et rollback d'une config | `catalogue-items-historique-de-versions-et-rollback-d-une-config` | 96.3 | = | moyenne | 98.4 | 100.0 | 95.0 | 90.0 |
