...
307 fonctionnalités — santé médiane 93.1 sur 307 mesurables.
...
| 10 | **Features (OGC API)** | 91.3 |
| 11 | **Fédération des données** | 85.2 |
...
| Automatisation | Construire un pipeline ETL no-code (graphe reader/transform/writer) | `automatisation-construire-un-pipeline-etl-no-code-graphe-reader-transform-writer` | 74.7 | = | haute | 91.2 | 100.0 | 81.6 | 10.0 |
| auth | Garde : refus de démarrer en mode mock d'authentification hors développement | `auth-garde-refus-de-demarrer-en-mode-mock-d-authentification-hors-developpement` | 75.6 | = | haute | 99.4 | — | — | 40.0 |
| Cartographie | Générer et publier des tuiles vectorielles pré-calculées (PMTiles) depuis PostGIS vers MinIO | `cartographie-generer-et-publier-des-tuiles-vectorielles-pre-calculees-pmtiles-de` | 28.0 | = | basse | 0.0 | — | — | 70.0 |
| Interne | Galerie interne de composants du kit UI | `interne-galerie-interne-de-composants-du-kit-ui` | 34.1 | = | basse | 31.8 | 0.0 | — | 80.0 |
...
| Carte | Catalogue de sources de couches (collections du cœur, couches raster moissonnées, tuiles 3D hébergées) | `carte-catalogue-de-sources-de-couches-collections-du-cur-couches-raster-moissonn` | 80.0 | = | haute | 100.0 | — | — | 50.0 |
| Carte | Persistance de la configuration de carte (basemap, vue, couches, terrain, mise en page d'impression) | `carte-persistance-de-la-configuration-de-carte-basemap-vue-couches-terrain-mise-` | 80.0 | = | haute | 100.0 | — | — | 50.0 |
| Déploiement | Démarrer une stack de développement complète en une commande | `deploiement-demarrer-une-stack-de-developpement-complete-en-une-commande` | 80.0 | = | haute | 100.0 | — | — | 50.0 |
...
| Catalogue | Mes vues (signets) | `catalogue-mes-vues-signets` | 70.2 | = | moyenne | 63.6 | — | — | 80.0 |
| Automatisation | Lancer l'exécution d'un pipeline à la demande | `automatisation-lancer-l-execution-d-un-pipeline-a-la-demande` | 80.7 | = | haute | 90.2 | 100.0 | 66.7 | 60.0 |
| Builder — Donnees | Panneau des sources de donnees (features/statistics/static) | `builder-donnees-panneau-des-sources-de-donnees-features-statistics-static` | 81.7 | = | haute | 82.8 | — | — | 80.0 |
| Automatisation | Importer un fichier géospatial (GeoJSON/CSV/GeoPackage/Shapefile zippé) comme nouvelle collection | `automatisation-importer-un-fichier-geospatial-geojson-csv-geopackage-shapefile-z` | 82.1 | = | haute | 91.7 | 100.0 | 62.5 | 70.0 |
| Fédération des données | Garde d'egress SSRF sur toute requête sortante du moteur de moissonnage | `federation-des-donnees-garde-d-egress-ssrf-sur-toute-requete-sortante-du-moteur-` | 82.2 | = | haute | 83.7 | — | — | 80.0 |
| Automatisation | Planifier l'exécution récurrente d'un pipeline (cron) | `automatisation-planifier-l-execution-recurrente-d-un-pipeline-cron` | 74.5 | = | moyenne | 84.2 | — | — | 60.0 |
| Builder — Widgets | Widget Carte (symbologie, popup, cross-filter, actions flyTo/highlight) | `builder-widgets-widget-carte-symbologie-popup-cross-filter-actions-flyto-highlig` | 83.4 | = | haute | 85.7 | — | — | 80.0 |
...
| Cartographie | Uploader une icône SVG personnalisée dans une bibliothèque d'icônes du tenant | `cartographie-uploader-une-icone-svg-personnalisee-dans-une-bibliotheque-d-icones` | 76.2 | = | moyenne | 95.5 | 100.0 | 50.0 | 50.0 |
...
| Carte | Visualisations deck.gl agrégées (heatmap/hexbin/column) | `carte-visualisations-deck-gl-agregees-heatmap-hexbin-column` | 78.9 | = | moyenne | 98.2 | — | — | 50.0 |
| Automatisation | Consulter l'historique des exécutions d'un pipeline | `automatisation-consulter-l-historique-des-executions-d-un-pipeline` | 79.9 | = | moyenne | 87.3 | 100.0 | 66.7 | 60.0 |
| Automatisation | Prévisualiser un pipeline jusqu'à un nœud donné, avant exécution complète | `automatisation-previsualiser-un-pipeline-jusqu-a-un-nud-donne-avant-execution-co` | 79.9 | = | moyenne | 87.3 | 100.0 | 66.7 | 60.0 |
| Builder — Variables | Variables typees d'app (string/number/bool/date/record/list) | `builder-variables-variables-typees-d-app-string-number-bool-date-record-list` | 80.0 | = | moyenne | 80.0 | — | — | 80.0 |
| Carte | Persistance de collectionId/pkColumn sur une couche 'feature' (URL GeoJSON) | `carte-persistance-de-collectionid-pkcolumn-sur-une-couche-feature-url-geojson` | 80.0 | = | moyenne | 100.0 | — | — | 50.0 |
| Fédération des données | Couches raster externes moissonnées (WMS/WMTS) proposées dans le sélecteur de couches de la carte | `federation-des-donnees-couches-raster-externes-moissonnees-wms-wmts-proposees-da` | 80.0 | = | moyenne | 100.0 | — | — | 50.0 |
| Navigation | Bandeau lecture seule (mode démo) | `navigation-bandeau-lecture-seule-mode-demo` | 80.0 | = | moyenne | 100.0 | — | — | 50.0 |
| Réseau/Sécurité | Ajouter des en-têtes de sécurité HTTP (HSTS, nosniff, frame-deny, referrer-policy) sur les routes exposées | `reseau-securite-ajouter-des-en-tetes-de-securite-http-hsts-nosniff-frame-deny-re` | 80.0 | = | moyenne | 100.0 | — | — | 50.0 |
| Réseau/Sécurité | Limiter le débit de requêtes contre le cœur et le shell exposés | `reseau-securite-limiter-le-debit-de-requetes-contre-le-cur-et-le-shell-exposes` | 80.0 | = | moyenne | 100.0 | — | — | 50.0 |
| Supervision | Détecter automatiquement un service en panne avant de router du trafic vers lui | `supervision-detecter-automatiquement-un-service-en-panne-avant-de-router-du-traf` | 80.0 | = | moyenne | 100.0 | — | — | 50.0 |
| Supervision | Observabilité packagée (Grafana + Prometheus + Loki + Tempo, dashboards pré-provisionnés) | `supervision-observabilite-packagee-grafana-prometheus-loki-tempo-dashboards-pre-` | 80.0 | = | moyenne | 100.0 | — | — | 50.0 |
...
| Administration | Administration des collections | `administration-administration-des-collections` | 87.4 | = | haute | 81.8 | 100.0 | — | 80.0 |
| Carte | Popup au clic sur une entité (liste de champs configurable) | `carte-popup-au-clic-sur-une-entite-liste-de-champs-configurable` | 87.4 | = | haute | 92.4 | — | — | 80.0 |
| Fédération des données | API STAC native (landing, conformance, collections, items, recherche cross-collection) | `federation-des-donnees-api-stac-native-landing-conformance-collections-items-rec` | 81.5 | = | moyenne | 88.7 | 100.0 | 87.5 | 40.0 |
| Cartographie | Uploader un DEM et le convertir en terrain 3D affichable sur la carte | `cartographie-uploader-un-dem-et-le-convertir-en-terrain-3d-affichable-sur-la-car` | 81.6 | = | moyenne | 95.9 | 100.0 | 87.5 | 30.0 |
| Builder — Donnees | Six grains temporels pour une source statistics | `builder-donnees-six-grains-temporels-pour-une-source-statistics` | 81.7 | = | moyenne | 82.8 | — | — | 80.0 |
| Déploiement | Déployer en production avec des images pré-construites publiées (aucun build sur l'hôte), hôte public piloté par une seule variable | `deploiement-deployer-en-production-avec-des-images-pre-construites-publiees-aucu` | 88.0 | = | haute | 100.0 | — | — | 70.0 |
| Administration | Lancer Martin/Titiler/Grafana depuis le shell derrière un gate cookie de courte durée | `administration-lancer-martin-titiler-grafana-depuis-le-shell-derriere-un-gate-co` | 82.5 | = | moyenne | 75.0 | 100.0 | 100.0 | 50.0 |
| users | Liste des utilisateurs du tenant, avec recherche et pagination | `users-liste-des-utilisateurs-du-tenant-avec-recherche-et-pagination` | 83.1 | = | moyenne | 64.3 | 100.0 | 83.3 | 90.0 |
| roles | Créer, éditer, supprimer un rôle sur mesure avec privilèges cochés par domaine | `roles-creer-editer-supprimer-un-role-sur-mesure-avec-privileges-coches-par-domai` | 83.3 | = | moyenne | 50.9 | 100.0 | 100.0 | 90.0 |
| Builder — Widgets | Outils de mesure/croquis sur le widget Carte, actifs seulement hors edition | `builder-widgets-outils-de-mesure-croquis-sur-le-widget-carte-actifs-seulement-ho` | 83.4 | = | moyenne | 85.7 | — | — | 80.0 |
| Carte | Légende de symbologie (couleurs par classe/catégorie) affichée sur la carte | `carte-legende-de-symbologie-couleurs-par-classe-categorie-affichee-sur-la-carte` | 83.4 | = | moyenne | 85.7 | — | — | 80.0 |
| Collections | Lire le détail d'une collection (avec emprise spatiale) | `collections-lire-le-detail-d-une-collection-avec-emprise-spatiale` | 89.2 | = | haute | 97.3 | 100.0 | 100.0 | 50.0 |
| Collections | Lire le schéma d'une collection (champs + pièces jointes) | `collections-lire-le-schema-d-une-collection-champs-pieces-jointes` | 89.2 | = | haute | 97.3 | 100.0 | 100.0 | 50.0 |
...
| Analytique | Agrégation de données d'une collection (groupBy multi-champs, mesures count/sum/avg/min/max/countDistinct/median/percentile/stddev, filtres, bbox/geomIntersects, bucket temporel, histogramme bins, échantillon) | `analytique-agregation-de-donnees-d-une-collection-groupby-multi-champs-mesures-c` | 89.7 | = | haute | 97.8 | 100.0 | 77.3 | 80.0 |
| Automatisation | Exporter une carte enregistrée (Bookmark) en image PNG ou PDF | `automatisation-exporter-une-carte-enregistree-bookmark-en-image-png-ou-pdf` | 84.8 | = | moyenne | 95.9 | 100.0 | 100.0 | 30.0 |
| Automatisation | Attacher un fichier (photo, document) à une entité depuis le widget Formulaire | `automatisation-attacher-un-fichier-photo-document-a-une-entite-depuis-le-widget-` | 89.9 | = | haute | 89.8 | 100.0 | 80.0 | 90.0 |
...
| Automatisation | Catalogue des rapports planifiés (/reports) | `automatisation-catalogue-des-rapports-planifies-reports` | 70.2 | = | basse | 63.6 | — | — | 80.0 |
| Fédération des données | Moissonnage : créer/lister/éditer/supprimer une source externe (STAC, ArcGIS FS, WMS, WFS, WMTS, CSW, OGC API - Records, CKAN) | `federation-des-donnees-moissonnage-creer-lister-editer-supprimer-une-source-exte` | 85.2 | = | moyenne | 84.1 | 100.0 | 91.7 | 60.0 |
...
| Features (OGC API) | SQL Lab : requête SQL en lecture seule sandboxée sur les collections visibles | `features-ogc-api-sql-lab-requete-sql-en-lecture-seule-sandboxee-sur-les-collecti` | 85.5 | -4.0 | moyenne | 97.5 | 91.7 | 77.3 | 70.0 |
...
| Features (OGC API) | Créer/modifier/supprimer une entité (OGC Part 4) | `features-ogc-api-creer-modifier-supprimer-une-entite-ogc-part-4` | 90.6 | = | haute | 94.2 | 100.0 | 77.3 | 90.0 |
...
| Automatisation | Inspecter les couches d'un GeoPackage/Shapefile avant de choisir laquelle importer | `automatisation-inspecter-les-couches-d-un-geopackage-shapefile-avant-de-choisir-` | 86.2 | = | moyenne | 91.9 | 100.0 | 62.5 | 90.0 |
...
| Cartographie | Héberger un tileset 3D (zip, ex. Cesium 3D Tiles) sans jamais l'extraire sur disque | `cartographie-heberger-un-tileset-3d-zip-ex-cesium-3d-tiles-sans-jamais-l-extrair` | 86.4 | = | moyenne | 96.5 | 100.0 | 90.0 | 50.0 |
...
| sharing | Partager une collection avec des groupes, avec court-circuit admin | `sharing-partager-une-collection-avec-des-groupes-avec-court-circuit-admin` | 86.7 | = | moyenne | 95.7 | 100.0 | 100.0 | 40.0 |
...
| Catalogue | Créer un nouvel élément (App/Dashboard/Map/Site/Dataset/Pipeline/Requête visuelle) | `catalogue-creer-un-nouvel-element-app-dashboard-map-site-dataset-pipeline-requet` | 91.3 | = | haute | 98.8 | — | — | 80.0 |
...
| Features (OGC API) | Lister/lire les entités d'une collection (bbox, filtres d'attribut, pagination) | `features-ogc-api-lister-lire-les-entites-d-une-collection-bbox-filtres-d-attribu` | 91.3 | = | haute | 96.8 | 100.0 | 77.3 | 90.0 |
...
| sharing | Partager un item avec des groupes (rôle lecteur/éditeur) et basculer sa visibilité publique | `sharing-partager-un-item-avec-des-groupes-role-lecteur-editeur-et-basculer-sa-vi` | 91.8 | = | haute | 96.5 | 100.0 | 79.2 | 90.0 |
| Cartes | Éditeur de carte (couches, fond de carte, terrain, caméra, impression, historique) | `cartes-editeur-de-carte-couches-fond-de-carte-terrain-camera-impression-historiq` | 91.8 | = | haute | 92.9 | 100.0 | — | 80.0 |
...
| Déploiement | Publier l'instance sur Internet sans ouvrir de port (tunnel Tailscale Funnel) | `deploiement-publier-l-instance-sur-internet-sans-ouvrir-de-port-tunnel-tailscale` | 88.0 | = | moyenne | 100.0 | — | — | 70.0 |
| Réseau/Sécurité | Bloquer les scripts/ressources non autorisés via une Content-Security-Policy | `reseau-securite-bloquer-les-scripts-ressources-non-autorises-via-une-content-sec` | 88.0 | = | moyenne | 100.0 | — | — | 70.0 |
...
| Catalogue/Items | Historique de versions et rollback d'une config | `catalogue-items-historique-de-versions-et-rollback-d-une-config` | 88.3 | = | moyenne | 98.4 | 100.0 | 95.0 | 50.0 |
| Collections | Enregistrer une table PostGIS existante comme collection | `collections-enregistrer-une-table-postgis-existante-comme-collection` | 88.4 | = | moyenne | 94.6 | 100.0 | 100.0 | 50.0 |
| Collections | Lister les tables PostGIS candidates à l'enregistrement | `collections-lister-les-tables-postgis-candidates-a-l-enregistrement` | 88.4 | = | moyenne | 94.6 | 100.0 | 100.0 | 50.0 |
| Collections | Modifier une collection (titre, publication, métadonnées ouvertes, champs pièces jointes) | `collections-modifier-une-collection-titre-publication-metadonnees-ouvertes-champ` | 88.4 | = | moyenne | 94.6 | 100.0 | 100.0 | 50.0 |
| Collections | Supprimer une collection | `collections-supprimer-une-collection` | 88.4 | = | moyenne | 94.6 | 100.0 | 100.0 | 50.0 |
| Fédération des données | Mode « copie » du moissonnage : matérialise le contenu distant en une collection propre au tenant (RLS, édition, etc.) | `federation-des-donnees-mode-copie-du-moissonnage-materialise-le-contenu-distant-` | 88.4 | = | moyenne | 94.0 | — | — | 80.0 |
...
| Automatisation | Exporter une App en conteneur Autoporté (mini-serveur embarqué) | `automatisation-exporter-une-app-en-conteneur-autoporte-mini-serveur-embarque` | 88.8 | = | moyenne | 96.0 | 100.0 | 100.0 | 50.0 |
| Collections | Créer une collection vide à schéma explicite | `collections-creer-une-collection-vide-a-schema-explicite` | 89.2 | = | moyenne | 97.3 | 100.0 | 100.0 | 50.0 |
...
| Administration | Accéder à la console MinIO depuis la page d'infrastructure admin | `administration-acceder-a-la-console-minio-depuis-la-page-d-infrastructure-admin` | 80.0 | = | basse | 100.0 | — | — | 50.0 |
...
| Analytique | Export (agrégat CSV/XLSX et entités GeoJSON/CSV/XLSX) d'un dataset ArcGIS Feature Service live | `analytique-export-agregat-csv-xlsx-et-entites-geojson-csv-xlsx-d-un-dataset-arcg` | 90.5 | = | moyenne | 88.7 | 100.0 | 91.7 | 80.0 |
| Analytique | Lecture live paginée/filtrée des entités d'un dataset ArcGIS Feature Service (pour rendu carte) | `analytique-lecture-live-paginee-filtree-des-entites-d-un-dataset-arcgis-feature-` | 90.5 | = | moyenne | 88.7 | 100.0 | 91.7 | 80.0 |
| Fédération des données | Exécution manuelle immédiate d'un moissonnage (bouton « Lancer ») | `federation-des-donnees-execution-manuelle-immediate-d-un-moissonnage-bouton-lanc` | 90.5 | = | moyenne | 88.7 | 100.0 | 91.7 | 80.0 |
...
| auth | Consultation du profil courant (identité, rôle, privilèges, capacités) | `auth-consultation-du-profil-courant-identite-role-privileges-capacites` | 93.8 | = | haute | 100.0 | 100.0 | 83.3 | 90.0 |
| users | Changer le rôle d'un utilisateur | `users-changer-le-role-d-un-utilisateur` | 93.8 | = | haute | 100.0 | 100.0 | 83.3 | 90.0 |
| users | Garde anti-lockout sur le changement de rôle d'un utilisateur (dernier titulaire admin.users.manage+admin.roles.manage) | `users-garde-anti-lockout-sur-le-changement-de-role-d-un-utilisateur-dernier-titu` | 93.8 | = | haute | 100.0 | 100.0 | 83.3 | 90.0 |
| Features (OGC API) | Exporter les entités brutes d'une collection (CSV/XLSX/GeoJSON/GPKG) | `features-ogc-api-exporter-les-entites-brutes-d-une-collection-csv-xlsx-geojson-g` | 91.1 | = | moyenne | 96.1 | 100.0 | 77.3 | 90.0 |
...
| Fédération des données | Créer un Dataset de plateforme à partir d'un Feature Service ArcGIS déjà moissonné (source live, sans copie) | `federation-des-donnees-creer-un-dataset-de-plateforme-a-partir-d-un-feature-serv` | 91.3 | = | moyenne | 98.8 | — | — | 80.0 |
| Features (OGC API) | Cascade de suppression des pièces jointes à la suppression d'une entité | `features-ogc-api-cascade-de-suppression-des-pieces-jointes-a-la-suppression-d-un` | 91.3 | = | moyenne | 96.8 | 100.0 | 77.3 | 90.0 |
| Features (OGC API) | Exporter un agrégat en CSV/XLSX | `features-ogc-api-exporter-un-agregat-en-csv-xlsx` | 91.3 | = | moyenne | 96.8 | 100.0 | 77.3 | 90.0 |
| Features (OGC API) | Landing page et déclaration de conformité OGC API Features | `features-ogc-api-landing-page-et-declaration-de-conformite-ogc-api-features` | 91.3 | = | moyenne | 96.8 | 100.0 | 77.3 | 90.0 |
...
| Automatisation | Voir les pièces jointes d'une entité dans le popup de la carte | `automatisation-voir-les-pieces-jointes-d-une-entite-dans-le-popup-de-la-carte` | 91.6 | = | moyenne | 95.4 | 100.0 | 80.0 | 90.0 |
...
| Builder — Copilote IA | Cinq operations client allowlistees pour le copilote | `builder-copilote-ia-cinq-operations-client-allowlistees-pour-le-copilote` | 92.0 | = | moyenne | 100.0 | — | — | 80.0 |
...
| Carte | Ajout d'une couche par URL GeoJSON externe | `carte-ajout-d-une-couche-par-url-geojson-externe` | 94.9 | = | haute | 98.2 | — | — | 90.0 |
| Carte | Options de gabarit d'impression : barre d'échelle et flèche du nord | `carte-options-de-gabarit-d-impression-barre-d-echelle-et-fleche-du-nord` | 84.8 | = | basse | 88.1 | — | — | 80.0 |
...
| auth | Accès anonyme en lecture aux items/collections publics | `auth-acces-anonyme-en-lecture-aux-items-collections-publics` | 95.2 | = | haute | 98.7 | — | — | 90.0 |
| auth | Authentification OIDC (Keycloak) avec provisioning JIT du compte, et mode mock réservé au développement | `auth-authentification-oidc-keycloak-avec-provisioning-jit-du-compte-et-mode-mock` | 95.2 | = | haute | 98.7 | — | — | 90.0 |
...
| Builder — Runtime | Redimensionnement d'un widget sur le canevas | `builder-runtime-redimensionnement-d-un-widget-sur-le-canevas` | 96.0 | = | haute | 100.0 | — | — | 90.0 |
| CI/Qualité | Empêcher toute dérive entre le schéma OpenAPI du cœur et les types TS générés côté shell | `ci-qualite-empecher-toute-derive-entre-le-schema-openapi-du-cur-et-les-types-ts-` | 96.0 | = | haute | 100.0 | — | — | 90.0 |
| CI/Qualité | Vérifier automatiquement lint/format/tests/couverture/E2E/build du shell à chaque push/PR | `ci-qualite-verifier-automatiquement-lint-format-tests-couverture-e2e-build-du-sh` | 96.0 | = | haute | 100.0 | — | — | 90.0 |
| CI/Qualité | Vérifier automatiquement lint/format/types/tests/couverture du cœur à chaque push/PR | `ci-qualite-verifier-automatiquement-lint-format-types-tests-couverture-du-cur-a-` | 96.0 | = | haute | 100.0 | — | — | 90.0 |
...
| Extensibilité | Découvrir les extensions actives et les capacités de l'instance depuis une page d'admin | `extensibilite-decouvrir-les-extensions-actives-et-les-capacites-de-l-instance-de` | 94.7 | = | moyenne | 100.0 | 100.0 | — | 80.0 |
| Automatisation | Définir une règle d'alerte de seuil sur un dataset | `automatisation-definir-une-regle-d-alerte-de-seuil-sur-un-dataset` | 94.8 | = | moyenne | 95.9 | 100.0 | 100.0 | 80.0 |
...
| Builder — Runtime | Grille responsive multi-breakpoints (sm/md/lg) | `builder-runtime-grille-responsive-multi-breakpoints-sm-md-lg` | 96.0 | = | moyenne | 100.0 | — | — | 90.0 |
| CI/Qualité | Auditer les dépendances npm à la recherche de vulnérabilités connues, avec allowlist | `ci-qualite-auditer-les-dependances-npm-a-la-recherche-de-vulnerabilites-connues-` | 96.0 | = | moyenne | 100.0 | — | — | 90.0 |
| CI/Qualité | Auditer les dépendances Python à la recherche de vulnérabilités connues | `ci-qualite-auditer-les-dependances-python-a-la-recherche-de-vulnerabilites-connu` | 96.0 | = | moyenne | 100.0 | — | — | 90.0 |
| CI/Qualité | Exécuter la suite E2E shell contre un vrai serveur Keycloak (OIDC réel, pas le mode mock) | `ci-qualite-executer-la-suite-e2e-shell-contre-un-vrai-serveur-keycloak-oidc-reel` | 96.0 | = | moyenne | 100.0 | — | — | 90.0 |
